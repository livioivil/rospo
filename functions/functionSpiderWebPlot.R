library(wesanderson)
colors=wes_palettes$Darjeeling #wes.palette(name= "Darjeeling",type = "continuous")(5)
cex.main=3
...
data.rows=rownames(data)
  nrows=nrow(data)
webplot(data, data.rows[1], main = main, col = colors[1], lwd=lwd,lty=lty,cex=cex,...)
for (i in 2:nrows)
  webplot(data, data.rows[i], add = T, col = colors[i], lwd=lwd,lty=lty,cex=cex,...)
par(new = TRUE)
par(mar = c(0, 0, 0, 0))
plot(0, type = "n", axes = F)
legend("bottomright", legend=rownames(data), 
       col = colors[1:nrows], rownames, bty = "n",
         cex=cex,
       lty = lty, lwd = lwd, ...       )
}